...
const GmailCompose = () => {
  ...
  const handleGenerateInitialDraft = async () => {
    ...
    try {
      ...
      if (selectedProfId) {
        payload.professor_id = parseInt(selectedProfId);
      } else if (to && to.trim()) {
        ...
        setTo(professors[0].email);
      }
      const res = await api.post('/ai/draft-email', payload);
      setSubject(res.data.subject);
      setBody(res.data.body);
      toast.success("Academic cold email drafted by Gemini!");
    } catch (err) {
      console.error("AI draft generation error:", err);
      toast.error(formatErrorMessage(err, "Failed to generate draft with Gemini."));
    } finally {
  ...
  const handleGenerateFollowUp = async (stageNum = 1) => {
    ...
    try {
      const payload = {
        ...
        previous_subject: subject || undefined,
        previous_body: body || undefined
      };
      if (selectedProfId) {
        payload.professor_id = parseInt(selectedProfId);
      } else if (to && to.trim()) {
      ...
      const res = await api.post('/ai/follow-up', payload);
      setSubject(res.data.subject);
      setBody(res.data.body);
      ...
      toast.error(formatErrorMessage(err, "Failed to generate follow-up."));
    } finally {
      setIsAiGenerating(false);
    }
  };
  ...
  const handleCopyToClipboard = () => {
    ...
    setCopied(true);
    toast.success("Copied to clipboard! Ready to paste into Gmail.", {
      icon: '📋',
      duration: 3500
    });
  ...
  const handleSaveDraft = async (options = {}) => {
    ...
    if (targetProf?.id) {
      ...
      if (!silent) toast.error("Please enter a recipient email address or select a professor.");
      return null;
    }

    try {
      const res = await api.post('/emails', payload);
      if (!silent) toast.success("Draft saved to CRM!");
      return res.data;
    } catch (err) {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const errMsg = formatErrorMessage(err, "SMTP dispatch failed.");
      toast.error(errMsg, { duration: 6000 });
      if (errMsg.toLowerCase().includes("smtp")) {
        toast("Tip: Click 'Open in Gmail' to send instantly without SMTP setup!", {
          icon: '💡',
          duration: 7000
        });
      }
    ...
    >
      ...
      {!isMinimized && (
        <div className="flex-1 flex flex-col overflow-hidden bg-white">
          ...
          <div className="px-4 py-3 bg-[#F6F8FC] border-t border-slate-200 flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-2">
              <button
                type="button"
                onClick={handleOpenInGmail}
                className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs shadow-md shadow-red-600/20 flex items-center gap-1.5 transition-all"
                title="Open directly in Gmail web with recipient, subject, and body pre-filled"
              >
              ...
              <button
                ...
                title="Send using configured SMTP credentials in Settings"
              >
                <Send className="w-3.5 h-3.5" />
                <span>{isSending ? "Sending..." : "Send (SMTP)"}</span>
              </button>
              ...
              <button
                ...
                title="Copy full email (Subject + Body) to clipboard to paste into any email client"
              >
